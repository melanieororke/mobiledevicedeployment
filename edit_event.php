...
if ($findiderrors == 0) {
    ...
    if (mysql_num_rows($memberresult) == 0) {
        echo '<h3>Update an Event</h3>
                        <p>Sorry, we cannot find that event (ID#' . $id . ') in our database. Please try again.</p>
                ';
    } else {
                ...
                foreach ($_POST AS $key => $value) {
                    ...
                    if (in_array($key, $allowedFields)) {
                        ...
                        if ($value == $row[$key]) {
                            ...
                            $query = "UPDATE shows SET $key='$value' WHERE id='$id'";
                            mysql_query($query) or die('Error updating event: ' . mysql_error() . ' ' . $query);
